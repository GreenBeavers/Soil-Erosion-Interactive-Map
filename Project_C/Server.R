...
generateRainfallbarplot <- function(filtered_data) {
  if (nrow(filtered_data) == 0) {
    return(NULL)
  }
  ggplot(Interpolated_Rainfall_Data, aes(x = Station, y = TMR)) +
    geom_bar(stat = "identity", fill = "steelblue") +
    labs(title = "Total Monthly Rainfall", x = "Stations", y = "Total Monthly Rainfall (mm)") +
    theme_minimal()
}

generateDTMbarplot <- function(filtered_data) {
  if (nrow(filtered_data) == 0) {
    return(NULL)
  }
leafletProxy("map") %>% #Generate scalebar for DTM
  clearControls() %>%
...
output$LC_barplot <- renderPlot({
 ...
 else if (input$point_colour == "Rainfall") { #If rainfall is selected in dropdown
   Interpolated_Rainfall_Data <- obsInBounds() #Get filtered data based on observations in view
  generateRainfallbarplot(Interpolated_Rainfall_Data) #generate the barplot based on filtered data
  
 } else if 
  (input$point_colour == "Elevation") {
    DTM_df <- obsInBounds()
    generateDTMbarplot(DTM_df)
   
  
  } else { #For other options in the dropdown menu
...
observe({
  ...
  colour_group <- LC_Grid_WGS84$Name
    ...
    colour_group <- DTM_df$Reprojected_Rasterized_DEM
    pal1 <- colorNumeric(palette = "Reds", domain = colour_group)
